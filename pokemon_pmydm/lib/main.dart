...
class _MyAppState extends State<MyApp> {
  ...
  fetchResults() async{
    var response = await await http.get(Uri.parse('https://pokeapi.co/api/v2/pokemon?limit=10'));
    var decodeJson = jsonDecode(response.body);
    setState(() {
      pokemons = decodeJson['results'];
    });
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        body: ListView.builder(itemCount: pokemons.length,
  ...
  void _navigateToNextScreen(BuildContext context){
    Navigator.of(context).push(MaterialPageRoute(builder: (context) => DetallePokemon()));
  }
}
